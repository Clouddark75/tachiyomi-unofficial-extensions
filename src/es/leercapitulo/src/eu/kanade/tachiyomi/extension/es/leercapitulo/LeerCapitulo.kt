...
class LeerCapitulo : ParsedHttpSource(), ConfigurableSource {
    ...
    override fun popularMangaFromElement(element: Element): SManga = SManga.create().apply {
        setUrlWithoutDomain(createMangaUrl(element.attr("abs:href")))
    ...
    override fun searchMangaParse(response: Response): MangasPage {
        val mangas = json.decodeFromString<List<MangaDto>>(response.body.string()).map {
            SManga.create().apply {
                setUrlWithoutDomain(createMangaUrl(it.link))
                title = it.label
    ...
    override fun latestUpdatesFromElement(element: Element): SManga = SManga.create().apply {
        val url = element.selectFirst(".media-body > a")!!.attr("abs:href")
        setUrlWithoutDomain(createMangaUrl(url))
    ...
    private fun createMangaUrl(urlString: String): String {
        val urlPath = urlString.substringAfter(baseUrl)
        val pathParts = urlPath.removeSurrounding("/").split("/")
    ...
    override fun pageListParse(document: Document): List<Page> {
        ...
        val sortedUrl = order?.map { urls[it.reversed().reversed().toInt()] } ?: urls
